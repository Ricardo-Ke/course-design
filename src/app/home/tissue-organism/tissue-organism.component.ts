...
@Component({
  selector: 'app-tissue-organism',
  templateUrl: './tissue-organism.component.html',
  styleUrls: ['./tissue-organism.component.css']
})
export class TissueOrganismComponent implements OnInit {
  ...
  private startRequest(): void {
    ...
    d3.queue()
      .defer(d3.json, this.webServiceUrl + 'statistics/tissues?size=50')
      .defer(d3.json, this.webServiceUrl + 'statistics/organisms?size=50') // geojson points
      .defer(d3.json, this.webServiceUrl + 'statistics/diseases?size=50') // geojson points
      .await(function(error: any, tissues: StatisticsDomainsDetail[], organisms: StatisticsDomainsDetail[], diseases: StatisticsDomainsDetail[]) {
        if (error) {
          ...
          if (self.retryLimitTimes <= 0) {
            self.outputErrorInfo(self.bubChartName);
            return;
          }
          
          self.outputGettingInfo(self.bubChartName);
          self.startRequest();
        }else {
          self.removeGettingInfo(self.bubChartName);
  ...
  private resetRadio(divWidthTemp: number): void {
    ...
    formdiv
      ...
      .attr("style", "position:absolute; bottom: 25px; left:" + (divWidthTemp - 266) / 2 + "px");
